...
- 2026-09-23 23:23（Asia/Shanghai）额度为 AMBER：五小时剩余 28%，每周剩余 65%。
- 第一问完整历史已准备快进到本地 `csh`；远端推送因 `github.com:443` 不可达而尚未发生。
...
1. GitHub 连通后执行 `git fetch origin csh`，验证快进关系，再执行 `git push origin csh:csh`。
